...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        ...
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'ghostfreak-config');
    } else if (platform === 'darwin') {
        // macOS: ~/Library/Application Support/ghostfreak-config
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'ghostfreak-config');
    } else {
        // Linux and others: ~/.config/ghostfreak-config
        configDir = path.join(os.homedir(), '.config', 'ghostfreak-config');
    }
